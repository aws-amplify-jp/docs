...
export const meta = {
  title: 'ストレージの設定',
  description: 'プロジェクト用に Amplify Storage を設定する',
  platforms: [
    'angular',
    'javascript',
    'nextjs',
...
保持されたバケットは引き続き S3 ストレージコストが発生します。予期しない請求を避けるため、不要になったら [AWS S3 コンソール](https://console.aws.amazon.com/s3/)または AWS CLI（`your-bucket-name` を置き換える：`aws s3 rb s3://your-bucket-name --force`）を使用して保持されたバケットを手動で削除してください。
...
{/* このドキュメントは設定を持っていますが、実際にはこのガイドではライブラリまたは UI コンポーネントのいずれも使用していません。これは代わりてはバックエンド接続ガイドにリンクしていますか？紹介では「ファイルの一覧表示、アップロード、ダウンロードを有効にする」と述べていますが、そうしていません。これを追加しましょうか？ */}

{/* JS には前提条件がありませんが、他にはあります。これは意図的ですか？*/}
