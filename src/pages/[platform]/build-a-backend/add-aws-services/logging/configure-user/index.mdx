...
export const meta = {
  title: 'ユーザー許可リストの設定',
  description: 'ユーザーごとにログレベルを設定する方法を学ぶ',
  platforms: [
    'swift',
    'android'
  ]
...
Amplifyで認証された各ユーザーは、独自のユニークなロギング設定を行うことができます。これは、ユーザーごとに問題をより詳細にデバッグできるようにするのに便利です。

## Amplify AuthでuserIDを取得する

必要に応じて、Amplify Authカテゴリを使用して特定のユーザーのuserIdを取得できます。また、Amazon Cognitoコンソールにアクセスしてユーザープールの`User ID`を確認することでもuserIdを取得できます。
...
<InlineFilter filters={['swift']}>
`AWSCloudWatchLoggingPlugin`の初期化と設定時に`UserLogLevel`のディクショナリを提供します。
