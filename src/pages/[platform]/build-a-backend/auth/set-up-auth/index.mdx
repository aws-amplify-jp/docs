...
export const meta = {
  ...
  description:
    'Amplify で認証用のバックエンドリソースをセットアップして接続する方法を学びます。',
  platforms: [
    'android',
    'angular',
    'flutter',
...
<Callout info>
プロジェクトが `React Native Authenticator` を使用したフェデレーションサインインをサポートする場合、`@aws-amplify/rtn-web-browser` パッケージも必要です：
...
<Callout warning>
Expo からネイティブライブラリとプラットフォーム依存関係を呼び出すには、関連プラットフォーム用のフォルダを生成するための prebuild コマンドを実行する必要があります。
