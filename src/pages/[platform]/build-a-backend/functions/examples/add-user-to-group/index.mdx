...
export const meta = {
  title: 'ユーザーをグループに追加',
  description:
    'Auth Post Authentication トリガーを使用して新しいユーザーを自動的にグループに追加',
  platforms: [
    'android',
    'angular',
    'flutter',
...
ユーザーが「確認」されるのは、アカウントを検証するときです。通常、これはユーザーが確認メールを介してメールを確認するときに発生します。Post confirmation ハンドラーは、フェデレーテッドサインイン（つまり、ソーシャルサインイン）ではトリガーされません。
