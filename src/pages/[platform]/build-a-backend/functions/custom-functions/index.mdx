...
export const meta = {
  title: 'カスタム関数',
  description:
    'Python、Golangなどの別のAWS Lambdaランタイムを使用してタスクを実行し、ワークフローをカスタマイズします。',
  platforms: [
    'android',
    'angular',
    'flutter',
...
**注意:** [フルスタックGitベースの環境](https://docs.amplify.aws/react/how-amplify-works/concepts/#fullstack-git-based-environments)は、デフォルトではDockerを使った関数のバンドリングをサポートしていません。詳細は[Dockerセクションをスキップ](#docker)してください。

</Callout>

<Callout info>

**注意:** `defineFunction`の以下のオプションはカスタム関数ではサポートされていません:
