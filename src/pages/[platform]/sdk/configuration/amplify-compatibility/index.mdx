...
export const meta = {
  ...
  description:
    'Amplify v2 で AWS SDK を使用する方法について説明します。',
  platforms: ['android']
};
...
<Callout warning>
AWS Mobile Client (com.amazonaws:aws-android-sdk-mobile-client) と Amplify Android v2 は相互に互換性がありません。Amplify v2 は AWS Mobile Client から別の形式に認証情報を移行し、AWS Mobile Client は認証情報を読み取ることができなくなります。この移行が行われた後に AWS Mobile Client が起動されると、Amplify v2 の認証情報も消去されます。
</Callout>

## Amplify V2 Auth と AWS Android SDK プラグインの使用

Amplify v2 では、AWS Android SDK ではなく、[AWS Kotlin SDK](https://aws.amazon.com/sdk-for-kotlin/) を使用することをお勧めします。既存の実装をサポートするために、このガイドでは Amplify v2 で AWS Android SDK プラグインを継続して使用する方法について説明します。

### AmplifyCredentialsProvider の作成

AWS Android SDK プラグインの多くは、カスタム `AWSCredentialsProvider` 実装を受け入れます。Amplify Android v2 を使用して認証情報を提供する独自の `AWSCredentialsProvider` を実装できます。
...
これで、`AWSCredentialsProvider` を受け入れるプラグインで `AmplifyCredentialsProvider` を使用できるようになりました。`AWSMobileClient.getInstance()` を AWSCredentialsProvider として使用する代わりに使用できます。


## AWS 設定情報の提供

Amplify v2 は `amplify_outputs.json` ファイルを使用しますが、AWS Android SDK は `awsconfiguration.json` ファイルを使用します。プロジェクトで Amplify v2 と AWS Android SDK の両方を使用している場合、リソースが同期されていることを確認することが重要です。Amplify CLI はこれらのファイル タイプの両方を生成および更新していますが、手動でカスタマイズした場合は、両方のファイルに適用する必要があります。

AWS Android SDK プラグインで設定情報が必要な場合は、`AWSConfiguration` クラスを継続して使用できます。
...
これはサポートされているプラグインの完全なリストではありません。`AWSCredentialsProvider` を受け入れ、AWS Mobile Client に依存しないプラグインはすべて機能します。

### S3 ストレージ (com.amazonaws:aws-android-sdk-s3)
...
### API Gateway で生成された Android SDK (aws-android-sdk-apigateway-core)
