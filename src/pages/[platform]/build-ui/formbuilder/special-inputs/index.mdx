...
export const meta = {
  title: '特別な入力を設定する',
  description: 'Amplify フォームビルダーの特別な入力フィールドにより、ユーザーはAmplifyの独自機能と対話できます',
  platforms: [
    'javascript',
    'react',
    'nextjs'
...
ファイルの上書きは同じパス内で同一のS3キーに対してのみ発生します。ファイルアップローダーの**ファイルレベルアクセス**が`private`または`protected`に設定されている場合、異なるユーザーがアップロードした同一ファイルは別々に保存されます。
