...
export const meta = {
  ...
  description: 'AWS Amplify Gen 2 を使用して Next.js Pages Router を始めましょう。',
  platforms: [
    'nextjs'
  ]
};
...
<Video src="/images/gen2/getting-started/react/deploy.mp4" description="ビデオ - 保存してデプロイ" />
...
<Video src="/images/gen2/getting-started/react/hosted-app.mp4" description="ビデオ - デプロイされた URL を表示" />

Amplify コンソールで、デプロイメントブランチ（この場合は **main**）をクリックして、左側のメニューで **Data** を選択し、**Data manager** を選択して、データベースに入力されたデータを確認します。

<Video src="/images/gen2/getting-started/amplify-console-data-manager.mp4" description="ビデオ - データマネージャー" />
...
<Video src="/images/gen2/getting-started/react/branch-details.mp4" description="ビデオ - デプロイメント" />
...
```
<Accordion title="amplify/data/resources.ts 全体を表示する">
...
<Video src="/images/gen2/getting-started/react/demo-delete.mp4" description="ビデオ - デモ" />
...
```
<Accordion title="amplify/auth/resources.ts 全体を表示する">
...
<Video src="/images/gen2/getting-started/react/demo-auth.mp4" description="ビデオ - 認証デモ" />
...
`npx ampx sandbox` コマンドは `npm run dev` と同時に実行する必要があります。クラウドサンドボックスはアプリケーションバックエンドの「localhost 相当」と考えることができます。
