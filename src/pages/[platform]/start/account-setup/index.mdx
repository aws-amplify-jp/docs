...
export const meta = {
  ...
  description:
    'AWSアカウントをセットアップし、Amplifyで使用するようにローカルで設定する方法を学びます。',
  platforms: [
    'android',
    'angular',
    'flutter',
...
**注意**: AWSアカウントとプロファイルがすでにローカルで設定されている場合、このガイドに従う必要はありません。設定済みのAWSプロファイルに`AmplifyBackendDeployFullAccess` IAMロールを追加してください。
...
<Accordion title="IAM Identity Center用語">
...
<Video src="/images/gen2/account-setup/sso-cloud-shell.mp4" description="ビデオ - CloudShellを開く" />
...
<Accordion title="手動セットアップの方が良いですか?" eyebrow="コンソールでのステップバイステップのウォークスルー">
...
<Video src="/images/gen2/account-setup/sso-reset-password.mp4" description="ビデオ - パスワードをリセット" />
...
<Accordion title="ブートストラップとは?" eyebrow="詳細情報">
