...
export const meta = {
  title: 'プラットフォーム設定',
  description: 'amplify-Flutterに必要なプラットフォーム固有の構成手順',
  platforms: [
    'flutter',
  ]
};
...
<Callout info>
`ios/Podfile`が存在しない場合は、`pubspec.yaml`に`amplify_flutter`を追加して`pub get`を実行してください。これでファイルが自動的に作成されます。
</Callout>
...
<Callout warning>
以下の手順は、Flutter 3.16以降で作成されたFlutterアプリを対象としています。アプリがバージョン3.16より前に作成された場合は、以下の手順に従う前に[こちらのガイド](https://docs.flutter.dev/release/breaking-changes/flutter-gradle-plugin-apply)に従ってGradleの宣言的プラグインブロックに移行してください。
</Callout>
...
<Callout info>
より高いバージョンのGradleまたはAndroid Gradleプラグインを使用したい場合は、[こちら](https://developer.android.com/build/releases/gradle-plugin#updating-gradle)の互換性マトリックスを参照してください。
</Callout>
...
<Callout info>
`macos/Podfile`が存在しない場合は、`pubspec.yaml`に`amplify_flutter`を追加して`pub get`を実行してください。これでファイルが自動的に作成されます。
</Callout>
...
この機能はAmplifyがmacOSでプラットフォームベストプラクティスとしてデータ保護キーチェーンを使用するため必須です。
