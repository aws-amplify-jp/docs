...
export const meta = {
  ...
  description: 'Amplify Storage API を、あなたのアカウント内の独自の S3 バケットに対して使用できます。',
  platforms: [
    'android',
    'angular',
    'flutter',
...
<Callout warning>
アプリケーションから手動で設定した S3 バケットに呼び出しを行うには、そのバケット用に [CORS ポリシー](/[platform]/build-a-backend/storage/extend-s3-resources/#for-manually-configured-s3-resources) も設定する必要があります。
</Callout>
