import { getCustomStaticPath } from '@/utils/getCustomStaticPath';

export const meta = {
  title: '競合解決',
  description: 'DataStore での競合解決の管理方法と設定方法について詳細に説明します。',
  platforms: [
    'flutter',
    'swift',
    'android',
    'javascript',
    'react-native',
    'angular',
    'nextjs',
    'react',
    'vue'
  ]
};

export const getStaticPaths = async () => {
  return getCustomStaticPath(meta.platforms);
};

export function getStaticProps(context) {
  return {
    props: {
      platform: context.params.platform,
      meta
    }
  };
}

import ios_maintenance from '/src/fragments/lib-v1/ios-maintenance.mdx';

<Fragments fragments={{ swift: ios_maintenance }} />

import flutter_maintenance from '/src/fragments/lib-v1/flutter-maintenance.mdx';

<Fragments fragments={{ flutter: flutter_maintenance }} />

import ios0 from '/src/fragments/lib-v1/datastore/native_common/conflict.mdx';

<Fragments fragments={{ swift: ios0 }} />

import android_maintenance from '/src/fragments/lib-v1/android-maintenance.mdx';

<Fragments fragments={{ android: android_maintenance }} />

import android1 from '/src/fragments/lib-v1/datastore/native_common/conflict.mdx';

<Fragments fragments={{ android: android1 }} />

import flutter2 from '/src/fragments/lib-v1/datastore/native_common/conflict.mdx';

<Fragments fragments={{ flutter: flutter2 }} />

import js3 from '/src/fragments/lib-v1/datastore/native_common/conflict.mdx';

<Fragments
  fragments={{
    javascript: js3,
    angular: js3,
    nextjs: js3,
    react: js3,
    vue: js3
  }}
/>

import reactnative0 from '/src/fragments/lib-v1/datastore/native_common/conflict.mdx';

<Fragments fragments={{ 'react-native': reactnative0 }} />