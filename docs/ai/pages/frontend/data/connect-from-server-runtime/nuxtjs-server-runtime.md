...
url: "https://docs.amplify.aws/react/frontend/data/connect-from-server-runtime/nuxtjs-server-runtime/"
---
...
> **Warning:** Amplify API の一部のみがサーバー側で使用可能であり、ライブラリが進化するにつれて、`amplify-apis.client` と `amplify-apis.server` はさらに異なる可能性があります。API 呼び出しをガードして、使用するコンテキストで API が利用可能であることを確認できます。例えば、`if (process.client)` を使用して、クライアント専用 API がサーバー上で実行されないようにすることができます。
