...
> **Warning:** Amplify API のサブセットのみがサーバー側で使用でき、ライブラリが進化すると `amplify-apis.client` と `amplify-apis.server` がさらに分岐する可能性があります。API 呼び出しをガードして、使用しているコンテキストで API が利用可能であることを確認できます（たとえば、クライアント専用 API がサーバー上で実行されないようにするために `if (process.client)` を使用できます）。
